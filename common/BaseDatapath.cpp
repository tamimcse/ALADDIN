...
void BaseDatapath::outputPerCycleActivity(
  ...
  int max_add = 0, max_cmp = 0, max_gep = 0, max_mul = 0, max_bit = 0, max_shifter = 0;
...
void BaseDatapath::writeSummary(std::ostream& outfile,
  ...
  if (summary.max_gep != 0)
    outfile << "Num of GetElemPtr (32-bit): " << summary.max_gep << std::endl;
...
void BaseDatapath::markNodeCompleted(
  ...
  if (!node->is_memory_op() && !node->is_branch_op())
    executedInstructionsPerCycle += "," + boost::lexical_cast<std::string>(node->fu_node_latency(user_params.cycle_time)) + ")";
  else
    executedInstructionsPerCycle += "," + boost::lexical_cast<std::string>(node->fu_node_latency(user_params.cycle_time)) + ")";  
